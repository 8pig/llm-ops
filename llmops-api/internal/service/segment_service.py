...
class SegmentService(BaseService):
    ...
    def create_segment(self, dataset_id: UUID, document_id: UUID, req: CreateSegmentReq, account: Account) -> Segment:
        ...
        try:
            ...
            logging.exception("新增文档片段内容发生异常, 错误信息: %s", str(e))
            if segment:
                self.update(
                    segment,
                    error=str(e),
    ...
    def update_segment(
        ...
        try:
            ...
            logging.exception("更新文档片段记录失败, segment_id: %s, 错误信息: %s", segment, str(e))
    ...
    def update_segment_enabled(
        ...
        with self.redis_client.lock(cache_key, LOCK_EXPIRE_TIME):
            try:
                ...
                logging.exception("更改文档片段启用状态出现异常, segment_id: %s, 错误信息: %s", segment_id, str(e))
                self.update(
                    segment,
                    error=str(e),
                    status=SegmentStatus.ERROR,
    ...
    def delete_segment(self, dataset_id: UUID, document_id: UUID, segment_id: UUID, account: Account) -> Segment:
        ...
        try:
            ...
            logging.exception("删除文档片段记录失败, segment_id: %s, 错误信息: %s", segment_id, str(e))
