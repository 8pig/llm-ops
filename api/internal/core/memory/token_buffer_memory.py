...
from langchain_core.messages import AnyMessage, BaseMessage, HumanMessage, AIMessage, trim_messages, get_buffer_string
...
def count_tokens_from_messages(messages: Sequence[BaseMessage]) -> int:
    """使用本地 tiktoken 计算消息 token 数，纯离线零网络请求"""
    try:
        enc = tiktoken.get_encoding("cl100k_base")
        return sum(
            len(enc.encode(str(msg.content or "")))
            for msg in messages
...
class TokenBufferMemory:
    ...
    def _safe_token_counter(self, messages: list[AnyMessage]) -> int:
        """安全的 token 计数器，使用本地 tiktoken 计算，纯离线零网络请求"""
